...
+ 1) Scope 提供 API (`scope.$watch()`) 来观察模型的变化;
+ 2) Scope 提供 API (`scope.$apply()`) 来实现在 AngularJS 框架之外更新数据时，也能让视图发生相应变化;
+ 3) Scopes 可以嵌套以限制对应用程序组件属性的访问，同时提供对共享模型属性的访问。嵌套的 scopes 为:
   + 子域(child scope): 继承了其父域的属性;
   + 孤立域(isolate scope): 不继承其父域的属性;
+ 4) Scopes 提供了评估表达式的上下文(context)。例如，在针对定义 `username` 属性的特定域进行评估之前，表达式 `{ {username} }` 是没有意义的。