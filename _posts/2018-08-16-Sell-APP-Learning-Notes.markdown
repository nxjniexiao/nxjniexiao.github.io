...
## 2. 制作图标字体
  ...
  5. 点击页面右下角的Download按钮，下载后解压。（下载之前可以点击左上角的Preference按钮，设置Font Name）
...
```
**tips：**`/* eslint-disable no-new */`用Vue实例化的时候，不需要赋值给变量。
...
+ 此外还有flex方法：<br>
  ...
  .clearfix
    ...
    &:after
      ...
      clear: both
  ```
其中 .detail-main的margin-top会导致其父元素.detail-wrapper的定位会向下移动，移动距离=margin-top。<br>
因为.clearfix样式中的：display: inline-block，避免了此现象。<br>
...
在鼠标点击+号按钮后，触发cartBallDrop() 触发方法，在执行了ball.show="true"后，会触发动画，依次执行beforeEnter-->enter-->afterEnter。<br>
...
显示顺序由上至下为：2>3>1（1没有继承父元素的z-index=200）<br>
...
### 11.2 star组件出现了问题：
传入参数相同的情况下，导入star组件，star组件中的星高度15px，最外层的div标签高度有的是15px，有的大于15px。（原因：star组件的父元素有：line-height：18px，这会影响star组件的高度。）<br>
...
food.vue（good.vue的子组件）中有个class=‘ratings-wrapper’，ratings.vue中最外层的class=‘ratings-wrapper’。<br>
在ratings.vue中，切换到good.vue，点击食物，激活food.vue后，food.vue中的class=‘ratings-wrapper’的元素，同样使用了ratings.vue中class=‘ratings-wrapper’的特性，从而导致排版bug。<br>
原因：未知。<br>
解决办法：ratings.vue中最外层的class=‘ratings-wrapper’改名。
